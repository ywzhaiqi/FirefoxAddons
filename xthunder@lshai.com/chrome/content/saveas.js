...
window.addEventListener("load", function() {
    ...
    var isAutoDown = function() {
        var exclude = xThunderPref.getValue("autoDownExclude");
        if (exclude) {
            var arr = exclude.split('\n');
            var excludeReg = arr.length > 0 ? new RegExp(arr.map(wildcardToRegExpStr).join("|")) : /^$/;
            if (excludeReg.test(dialog.mLauncher.source.spec)) {
                return false;
            }
        }
        return true;
    ...
    if (extExists && remExt > 0 && isAutoDown()) {
        download();
        return;
    }
